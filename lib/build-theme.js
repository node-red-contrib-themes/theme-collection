...
async function buildTheme(themeName) {
    ...
    async function buildCSS(themeName) {
        ...
        async function jQueryAssets(compiledSass) {
            ...
            const jQueryAssetColorHover = khroma
                .toHex(khroma.opacify(compiledSass.match(/secondary-text-color-hover: (.*);/)[1], 1))
                .replace('#', '')
            const jQueryAssetColorActive = khroma
                .toHex(khroma.opacify(compiledSass.match(/secondary-text-color-active: (.*);/)[1], 1))
                .replace('#', '')
            const jQueryAssetColor = khroma
                .toHex(khroma.opacify(compiledSass.match(/form-text-color: (.*);/)[1], 1))
                .replace('#', '')
            ...
            async function jQueryAssetToBase64() {
                const jQueryAssetColorHoverBase64 = fs
                    .readFileSync(path.join(jQueryAssetsDir, `ui-icons_${jQueryAssetColorHover}_256x240.png`))
                    .toString('base64')
                const jQueryAssetColorActiveBase64 = fs
                    .readFileSync(path.join(jQueryAssetsDir, `ui-icons_${jQueryAssetColorActive}_256x240.png`))
                    .toString('base64')
                const jQueryAssetColorBase64 = fs
                    .readFileSync(path.join(jQueryAssetsDir, `ui-icons_${jQueryAssetColor}_256x240.png`))
                    .toString('base64')
    ...
    async function buildMonacoTheme(themeName) {
        ...
        if (typeof monacoTheme === 'object') {
            for (const entry of Object.entries(monacoTheme.colors)) {
                ...
                if (!monacoExposedColors.includes(_key) || entry[1] === null) {
                    delete monacoTheme.colors[_key]
                }
            }
        }
