...
function buildTheme() {
    ...
    function vsCodeToSCSS() {
        ...
        for (const [colorMapKey, colorMapValue] of Object.entries(colorMap)) {
            ...
            if (!found) {
                log(styleText('yellow', `A color value for $${colorMapKey} was not found`))
            }
        }
    ...
    log('')
    log(
        `An initial version of the ${styleText('bold', themeName)} theme was created in ${styleText('bold', themeSourceDir)}`
    )
    log('')
