...
const themeDir = path.join(rootDir, 'src/themes', themeName)
...
function cleanVSCodeTheme() {
    const vsCodeThemeFile = path.join(themeDir, 'theme-vscode.json')
    ...
    fs.writeFileSync(vsCodeThemeFile, JSON.stringify(vsCodeTheme))
}

//* VSCode's sort only does it for first level keys
//? Should we do it like we're doing it now?
...
function sortVSCodeTheme() {
    ...
    vsCodeTheme.colors = Object.keys(vsCodeTheme.colors)
        .sort()
        .reduce((obj, key) => {
            obj[key] = vsCodeTheme.colors[key]
            return obj
        }, {})

    vsCodeTheme.tokenColors = vsCodeTheme.tokenColors.sort((a, b) =>
        a.scope > b.scope ? 1 : b.scope > a.scope ? -1 : 0
    )

    fs.writeFileSync(vsCodeThemeFile, JSON.stringify(vsCodeTheme))
}

//? Can we format the file using biomejs after it's sorted?
...
function buildTheme() {
    ...
    function vsCodeToCustomCSS() {
        const themeCustomCSSFile = path.join(themeDir, 'theme-custom.css')
    ...
    function vsCodeToMermaid() {
        const themeMermaidFile = path.join(themeDir, 'theme-mermaid.json')
    ...
    function vsCodeToMonaco() {
        ...
        const monacoThemeFile = path.join(themeDir, 'theme-monaco.json')
        ...
        fs.writeFileSync(monacoThemeFile, JSON.stringify(monacoTheme))
    }

    function vsCodeToSCSS() {
        const themeScssFile = path.join(themeDir, 'theme.scss')
    ...
    log('')
    log(`An initial version of the ${chalk.bold(themeName)} theme was created in ${chalk.bold(themeDir)}`)
    log('')
