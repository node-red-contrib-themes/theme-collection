;(async () => {
    ...
    await runner('npm install', noderedDir, 'Installing Node-RED dependencies', true)
    ...
    if (!fs.existsSync(projectsDir)) {
        ...
        await runner('git pull', themeDevProjectDir, 'Updating theme development project local repository')
    }
    // Notice the quotes around themeDevProjectDir to handle paths with spaces
    await runner(`npm install "${themeDevProjectDir}"`, userDir, 'Installing project dependencies', true)
    ...
    async function runner(cmd, workingDir, prompt, hasSuffixText = false) {
        ...
        try {
            const suffix = `- ${chalk.yellow('This may take a while, please be patient')}`
