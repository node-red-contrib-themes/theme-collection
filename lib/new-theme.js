...
if (!/(?=^.{3,}$)^[a-z0-9]+(-[a-z0-9]+)*$/.test(themeName)) {
    log('')
    error('Theme name not valid')
    log('')
    log('Rules for theme names:')
    log('')
    log(`- It can only contain lowercase chars ('a-z'), numbers ('0-9'), and dash ('-')`)
    log(`- It cannot begin or end with non-alphanumeric chars`)
...
fs.cpSync(path.join(rootDir, 'src/template/'), themeSourceDir, { recursive: true })

function showUsageAndExit(exitCode) {
    log('')
    log('Usage: npm run new-theme theme-name')
